...
* sgp4.cs
*
* this file contains the Algorithms for SGP4 Orbit Model
*
* Part of the Code for Orbit-calculation is based on the SPACETRACK REPORT NO. 3
...
namespace MARRSS.SGP4
{
    class Sgp4
    {
        ...
        private TLE.Tle tleElementData;
        private Sgp4Rec satCalcData;
	    ...
	    */
        ...
        public Sgp4(TLE.Tle data, int gravConst)
        {
            setGrav(gravConst);
            ...
            SatDateTime satTime = new SatDateTime(satCalcData.rec_epochyr,
                satCalcData.rec_epochdays);
        ...
        public void runSgp4Cal(TimeDate starttime, TimeDate stoptime, double step)
        {
        ...
        Sgp4Data calcSgp4()
        {
	        ...
	        emsq = em * em;
	        temp = 1.0 - emsq;
	        omegam = modfunc(omegam, Constants.twoPi);
	        argpm = modfunc(argpm, Constants.twoPi);
	        xlm = modfunc(xlm, Constants.twoPi);
	        mm = modfunc(xlm - argpm - omegam, Constants.twoPi);
	        ...
	        if (satCalcData.neo.neo_method == 2) {
		        ...
		        if (satCalcData.rec_xincp < 0.0)
		        {
			        ...
			        satCalcData.rec_omegap = satCalcData.rec_omegap + Constants.pi;
			        satCalcData.rec_argpp = satCalcData.rec_argpp - Constants.pi;
		        }

		        // Another eccentricity check
		        if ((satCalcData.rec_ep < 0.0) || (satCalcData.rec_ep > 1.0))
		        {
                    satCalcData.rec_error = 3;
			        // System.out.println("ERROR eccentricity out of bounds
		        }
	        }
	        ...
	        u = modfunc(xl - satCalcData.rec_omegap, Constants.twoPi);
	        eo1 = u;
	        tem5 = 9999.9;
	        ktr = 1;
        ...
        { /* --------------------- local variables ------------------------ */
	        ...
	        if (init == 0) {
		        ...
		        if (satCalcData.rec_xincp >= 0.2) {
			        ...
			        alfdp = alfdp + dalf;
			        betdp = betdp + dbet;
			        satCalcData.rec_omegap = modfunc(satCalcData.rec_omegap, Constants.twoPi);
			        ...
			        if (Math.Abs(xnoh - satCalcData.rec_omegap) > Constants.pi)
			        if (Math.Abs(xnoh - satCalcData.rec_omegap) > Constants.pi)
			        if (satCalcData.rec_omegap < xnoh)
				        satCalcData.rec_omegap = satCalcData.rec_omegap + Constants.twoPi;
			        else
				        satCalcData.rec_omegap = satCalcData.rec_omegap - Constants.twoPi;
			        satCalcData.rec_mp = satCalcData.rec_mp + pl;
    ...
    * modified by Nikolai Reed 2015
	----------------------------------------------------------------------------*/
						        ...
						        double xfact, double xlamo, double no)
        {
	        ...
	        dndt = 0.0;
	        theta = modfunc(gsto + tc * rptim, Constants.twoPi);
	        em = em + dedt * t;
	        inclm = inclm + didt * t;
	        argpm = argpm + domdt * t;
     ...
     *
     ----------------------------------------------------------------------------*/
        ...
        if ((omeosq >= 0.0) || (satCalcData.rec_no >= 0.0))
        {
            ...
            if ((2 * Constants.pi / satCalcData.rec_no) >= 225.0)
            {
     ...
     *
                                ...
                                double inclp, double omegap, double np) 
     {
        ...
        xnodce = modfunc(4.5236020 - 9.2422029e-4 * day, Constants.twoPi);
        ...
        satCalcData.dso.dso_zmol = modfunc(4.7199672 + 0.22997150 * day - gam,
                Constants.twoPi);
        satCalcData.dso.dso_zmos = modfunc(6.2565837 + 0.017201977 * day, Constants.twoPi);
    ...
    private void newtonm(double ecc, double m, double e0, double nu) {
        ...
        if ((ecc - 1.0) > small) {
            /* ------------ initial guess ------------- */
            if (ecc < 1.6)
                if (((m < 0.0) && (m > - Constants.pi)) || (m > Constants.pi))
                    e0 = m - ecc;
                else
                    e0 = m + ecc;
            else if ((ecc < 3.6) && (Math.Abs(m) > Constants.pi)) // just edges)
     ...
     private void deepSpaceinit(double cosim, double emsq, double argpo, double s1,
        ...
        if (satCalcData.dso.dso_irez == 1) {
            ...
            satCalcData.dso.dso_del1 = satCalcData.dso.dso_del1 * f311 * g310 * q31
                    * aonv;
            satCalcData.dso.dso_xlamo = modfunc(mo + omegao + argpo - theta, Constants.twoPi);
        ...
        public double gstime(double jdut1)
        {
	        ...
	        temp = modfunc(temp * Constants.toRadians / 240.0, Constants.twoPi); 
